...
PixelShader* GetPixelShader(ID3D11Device* aDevice, const std::string& aPath)
{
	...
	std::string binaryPath = aPath.substr(0, aPath.size() - BasePath.extension().string().size()) + "_ps.cso";
	if (LoadedPixelShaders().count(binaryPath) == 0)
	{
		...
		{
			SYSINFO("Loading pixelshader: " + aPath + " From File");
		...
		else
		{
			...
			SYSINFO("(Re)compiling pixelshader: " + aPath);
			result = D3DCompileFromFile((std::wstring(aPath.begin(), aPath.end())).c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "pixelShader", "ps_5_0", flags, 0, &compiledShader, &errorBlob);
			if (SUCCEEDED(result))
			{
			...
			if (errorBlob)
			{
				SYSERROR("Could not compile pixel shader",aPath);
...
{
	std::experimental::filesystem::path BasePath(aPath);
	std::string binaryPath = aPath.substr(0, aPath.size() - BasePath.extension().string().size()) + "_vs.cso";
	...
	if (LoadedVertexShaders().count(binaryPath) == 0)
	{
		...
		if (csoFile && FileAge(binaryPath) > FileAge(aPath) && false)
		{
			SYSINFO("Loading vertexshader: " + aPath + " From File");
			std::streamsize binarySize = csoFile.tellg();
			csoFile.seekg(0, std::ios::beg);
			blob.resize(binarySize);
			csoFile.read(blob.data(), binarySize);
		...
		else
		{
			...
			SYSINFO("(Re)compiling vertexshader: " + aPath);
			result = D3DCompileFromFile((std::wstring(aPath.begin(), aPath.end())).c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "vertexShader", "vs_5_0", flags, 0, &compiledShader, &errorBlob);

			if (errorBlob)
			{
				SYSERROR((char*)errorBlob->GetBufferPointer(), BasePath.string());
				SYSERROR("Could not compile pixel shader", BasePath.string());
				errorBlob->Release();
				return nullptr;
			}
			if (SUCCEEDED(result))
		...
		if (!globalIsFileWatched[aPath])
		{
			ShaderFileWatcher.RegisterCallback(aPath, std::bind(ReloadVertexShader, aDevice, std::placeholders::_1));
			globalIsFileWatched[aPath] = true;
		}
#endif // USEFILEWATHCER
#ifndef  _RETAIL
...
{
	std::string binaryFile = aPath.substr(0, aPath.length() - std::experimental::filesystem::path(aPath).extension().string().length()) + "_ps.cso";
	...
	if (LoadedPixelShaders().find(binaryFile) != LoadedPixelShaders().end())
	{
		...
		PixelShader* end = GetPixelShader(aDevice, aPath);
		if (end)
		{
			start->ReleaseShader();
...
{
	std::string binaryFile = aPath.substr(0, aPath.length() - std::experimental::filesystem::path(aPath).extension().string().length()) + "_vs.cso";
	...
	if (LoadedVertexShaders().find(binaryFile) != LoadedVertexShaders().end())
	{
		...
		std::vector<char> _;
		VertexShader* end = GetVertexShader(aDevice, aPath, _);
		if (end)
		{
			start->ReleaseShader();
			start->myShader = end->myShader;
...
bool LoadPixelShader(ID3D11Device* aDevice, std::string aFilePath, ID3D11PixelShader*& aShaderOutput)
{
	...
	HRESULT hr = D3DCompileFromFile((std::wstring(aFilePath.begin(), aFilePath.end())).c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "pixelShader", "ps_5_0", flags, 0, &shaderBlob, &errorBlob);
...
bool LoadGeometryShader(ID3D11Device* aDevice, std::string aFilePath, ID3D11GeometryShader*& aShaderOutput)
{
	...
	HRESULT hr = D3DCompileFromFile((std::wstring(aFilePath.begin(), aFilePath.end())).c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "geometryShader", "gs_5_0", flags, 0, &shaderBlob, &errorBlob);
...
bool LoadVertexShader(ID3D11Device* aDevice, std::string aFilePath, ID3D11VertexShader*& aShaderOutput, void* aCompiledOutput)
{
	UINT flags = D3DCOMPILE_ENABLE_STRICTNESS;
#ifdef _DEBUG 
	flags |= D3DCOMPILE_DEBUG;
	...
	HRESULT hr = D3DCompileFromFile((std::wstring(aFilePath.begin(), aFilePath.end())).c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "vertexShader", "vs_5_0", flags, 0, shaderBlob, &errorBlob);
...
bool CompilePixelShader(ID3D11Device* aDevice, std::string aData, ID3D11PixelShader*& aShaderOutput)
{
	...
	HRESULT hr = D3DCompile(aData.c_str(), aData.length(), NULL, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "pixelShader", "ps_5_0", flags, 0, &shaderBlob, &errorBlob);
...
bool CompileGeometryShader(ID3D11Device* aDevice, std::string aData, ID3D11GeometryShader*& aShaderOutput)
{
	...
	HRESULT hr = D3DCompile(aData.c_str(), aData.length(), NULL, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "geometryShader", "gs_5_0", flags, 0, &shaderBlob, &errorBlob);
...
bool CompileVertexShader(ID3D11Device* aDevice, std::string aData, ID3D11VertexShader*& aShaderOutput, void* aCompiledOutput)
{
	...
	HRESULT hr = D3DCompile(aData.c_str(), aData.length(), NULL, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "vertexShader", "vs_5_0", flags, 0, shaderBlob, &errorBlob);
